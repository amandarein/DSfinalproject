...
## Error Rates
Next, we will calculate the error rate for each individual tree. We will use these 1,000 error rates to visualize the overall performance of the random forest. 
```{r, message=FALSE, echo = FALSE, warning=FALSE}

#View(as.data.frame(bot_rf$err.rate))

err.rate <- as.data.frame(bot_rf$err.rate) # for each tree
...
Now that we have a base random forest, we can tune the parameters to create an optimized model. In this situation, Twitter wants to correctly identify as many bots as possible so they can remove the accounts. Therefore, we want to focus on correctly identifying bots (the positive class). However, Twitter does not want to incorrectly identify a real account as a bot and delete a human's account, so we would like to see the false positive rate decrease as well. 

We would like to use the number of trees that would minimize both the bot error and OOB error. Looking at the error rates, we will build another random forest with 20 trees using the same mytry value and training data. 
```{r, message=FALSE, echo = FALSE, warning=FALSE}
...
We have the confusion matrices for the first and second random forest, respectively. The optimized model does a better job at predicting bots, but the false positive rate also increased. About 30 more bots were correctly identified but about 30 more humans were mistaken as bots. Since we want to focus on identifying bots, we must accept the trade-off.
```{r, message=FALSE, echo = FALSE, warning=FALSE}
...
# Evaluate Model {.tabset}
## Confusion Matrix
Based on the confusion matrix below, our model does a good job at prediction. The accuracy is 87.66% and the Kappa value is 0.7531. The Sensitivity and Specificity are 88.21% and 87.1%, respectively. We want higher values for these metrics, so we are happy with these results. 
```{r, message=FALSE, echo = FALSE, warning=FALSE}
...
View(as.data.frame(op_bot_rf$importance))

```

## Error Rate
The error rate for using the optimized model for prediction is 12.34%. We want to minimizde the errors, so we are pleased with the low error rate.
```{r, message=FALSE, echo = FALSE, warning=FALSE}
